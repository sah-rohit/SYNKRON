...
export async function GET(req: NextRequest) {
  ...
  if (process.env.DATABASE_URL) {
    ...
    return NextResponse.json({ success: true, sessions: sessions.map(s => ({ id: s.token.slice(0, 8), deviceInfo: s.deviceInfo, ipAddress: s.ipAddress, location: null, createdAt: new Date(s.expiresAt - 86400000).toISOString(), isCurrent: s.token === token })) });
  }
}
