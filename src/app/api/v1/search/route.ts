...
function getDemoResults(query: string) {
  ...
  // Simple keyword relevance for demo — filter() always returns an array so || is unreachable
  ...
  return filtered.length ? filtered.slice(0, 3) : demos.slice(0, 2);
}
